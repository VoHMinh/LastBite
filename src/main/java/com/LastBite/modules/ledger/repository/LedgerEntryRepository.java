...
public interface LedgerEntryRepository extends JpaRepository<LedgerEntry, UUID> {
    ...
    @Query("""
        ...
        WHERE a.accountType = :accountType
          ...
          AND e.availableAt <= :now
          AND NOT EXISTS (
              SELECT r.id FROM RefundRequest r
              WHERE r.order = o
                AND r.status IN :openRefundStatuses
          )
        ORDER BY st.id, e.createdAt
    """)
    List<LedgerEntry> findUnsettledSettleableMerchantEntries(
            @Param("accountType") LedgerAccountType accountType,
            @Param("now") Instant now,
            @Param("openRefundStatuses") Collection<RefundStatus> openRefundStatuses);
}
